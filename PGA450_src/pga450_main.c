...
//when enabled: combines burst, capture, compare, and printing results to terminal on single command.
#define BurstCaptureComparePrint; 
...
void command1 (void){       // Do distance Measurment with predefined driver/receive settings or UART entered
    // Valid data for command1 is only  0 - Liten mode ; 
    //                                  1 - Short Distance Detection; 
    //                                  2 - Long Distance Detection;
    if (UART_data[3] > 2)   return;     
...
void burst_cap_comp (void){     // Burst, Capture, Compare
    ...
    {
        EN_CTRL = 0x80;     // Log Free Running Timer only. Used for Listen only mode, or triangulation.    
    }
